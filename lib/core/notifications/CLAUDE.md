...
| `showLeadNuevoBotNotification(WebSocketMessage)` | Parsea con `NuevoLeadBotPayload` (6 campos: idLead, codAsesor, nombreCliente, numero, idChatCab, idNumero). Notificación con título fijo "Nuevo lead derivado por el bot" y body fijo (no usa datos del cliente) + botones "Ver lead" (`ver_negociacion_bot`) y "Abrir conversación" (`abrir_conversacion_bot`) — los dos navegan. ID = idLead (reemplaza). |
