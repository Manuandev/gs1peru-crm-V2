...
class SolicitudRemoteDatasource {
  ...
  Future<CrudResult> guardarSolicitud({
    ...
    // Inversión/IGV/Importe total YA calculados por
    // calcularTotalesSolicitud() (solicitud_guardar_helper.dart) — el mismo
    // cálculo que se muestra en pantalla, así lo guardado nunca se
    // desincroniza de lo mostrado. Van tal cual como DC_IMPORTE/DC_IGV/
    ...
    // Los 3 montos de cabecera llegan ya calculados (ver `totales` arriba)
    // — las reglas (solo Pagantes, descuento ya repartido en cada importe,
    // total fijado al precio pactado con la solicitud completa, IGV = Total
    // − Inversión, montos guardados respetados) viven ahora en
    // calcularTotalesSolicitud(), no acá.
    final dcImporte = totales.inversion;
    final dcIgv = totales.igv;
    final dcImporteTotal = totales.importeTotal;
    ...
    final cabecera = <String>[
      ...
      solicitante.cargo, // 13 CARGO_SOL — texto libre, ya no id de catálogo
    ...
    final detalle = participantes
        .map((p) {
          ...
          return [
            ...
            p.correo, // CORREO
            p.celular, // CELULAR
            p.cargo, // CARGO — texto libre, ya no id de catálogo
